...
def write_2d_tif(extreme_index_name, output_folder, mode, scenario, year, array):
    ...
    out_tif.SetProjection(srs.ExportToWkt())  # Define the output coordinate system as WGS84
...
def mission(m):
    ...
    count = 0
    input_folder = "G:\\weather extreme\\Revision\\heatwave\\humid_heatwave_begin\\"
    output_folder = "F:\\Heatwave\\humid_heatwave_result\\"
    log_folder = "F:\\Heatwave\\humidheatwavebegin_log\\"
    ...
    for s in tqdm(scenarios):
        ...
        for y in years:
            try:
                if "HumidHeatWave" + '_' + m + '_' + \
                    s + "_" + str(y) + '.tif' in os.listdir(output_folder):
                        continue
                if y == 1950 or y == 2015:
                    result = gdal.Open(input_folder + f"HumidHeatWave_{m}_{s}_{y}.tif").ReadAsArray()
                    write_3d_tif("HumidHeatWave", output_folder, m, s, y, result)
                else:
                    hw = gdal.Open(input_folder + f"HumidHeatWave_{m}_{s}_{y}.tif").ReadAsArray().astype(np.float32)

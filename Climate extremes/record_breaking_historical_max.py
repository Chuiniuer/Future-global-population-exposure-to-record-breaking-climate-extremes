...
if __name__ == "__main__":
    ...
    out_folder = "G:\\weather extreme\\Revision\\heat(tasmax)\\WD_his_record_extreme_threshold\\"
    ...
    for m in tqdm(mode_list):
        for i in indices:
            fs = []
            for f in files:
                if (m in f) and (i in f) and (senario in f):
                    fs.append(f)
            for j in range(len(fs)):
                try:
                    if j == 0:
                        arr = np.expand_dims(get_arr_from_tif(fs[j]), axis=0)
                    else:
                        arr = np.concatenate((arr, np.expand_dims(get_arr_from_tif(fs[j]), axis=0)), axis=0)
                except:
                    print(f"{m} {i} {fs[j]} has encountered a problem.")
            print(arr.shape)
            write_2d_tif("WD", out_folder, m, index.get_maximum_indices(arr))
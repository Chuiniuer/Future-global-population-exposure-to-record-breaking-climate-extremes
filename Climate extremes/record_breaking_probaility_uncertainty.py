...
class IndexExtreme:
    ...
    def get_median(self, array):
        return np.percentile(array, 50, axis=0)

if __name__ == "__main__":
    # input_folder = "E:\\weather extreme\\extereme_heat\\"
    # mode_list = []
    ...
    input_folder = "G:\\weather extreme\\Revision\\heat(tasmax)\\WD_record_breaking_prob\\"
    indices = ["WD"]
    senarios = ["ssp126", "ssp245", "ssp370", "ssp585"]
    periods = ["lat", "mid"]
    index = IndexExtreme()
    out_folder = "G:\\weather extreme\\Revision\\heat(tasmax)\\WD_record_breaking_uncertain\\"
    for i in indices:
        for s in senarios:
            for p in periods:
                ...
                write_result(out_folder + '\\' + i + "_" + p + "_" + s + '_median.tif', result)
                #90th
                result = index.get_90th_percen(arr)
                result[nodatamask==1] = -9999
                write_result(out_folder + '\\' + i + "_" + p + "_" + s + '_90th.tif', result)
                #10th
                result = index.get_10th_percen(arr)
                result[nodatamask==1] = -9999
                write_result(out_folder + '\\' + i + "_" + p + "_" + s + '_10th.tif', result)
